...
cursor = db["US_Accidents"].find().limit(1000000) # Definir quantidade de dados para tratamento
df =  pd.DataFrame(list(cursor), columns=colunas_desejadas.keys())
...
df["Data_Hora_Inicio"] = pd.to_datetime(df["Data_Hora_Inicio"], format="%Y-%m-%d %H:%M:%S")
df["Data_Hora_Fim"] = pd.to_datetime(df["Data_Hora_Fim"], format="%Y-%m-%d %H:%M:%S")
# Cálculo da duração do acidente em horas
...
print(f"{len(registros)} registros inseridos com sucesso na coleção 'Acidentes - Tratados'.")